...
function mostrar(result) {
    const content = result.map((country)=> {
        const flagURL = countriesFlags[country].normal;
        const countryName = countriesFlags[country].alias || country;
        return `<li onClick="selectInput('${countryName}')">
...
function selectInput(countryName) {
    console.log(countryName)
    inputBox.value = countryName;
    resultsBox.innerHTML = '';
}

function search() {
    fetchCountryData(inputBox.value)
    .then(data => {
        window.location.href = `informacoes_pais.html?pais=${inputBox.value}`;
    })
    .catch(error => {
      alert("Opss! País não encontrado.")
    });
}
