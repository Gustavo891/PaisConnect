const countriesFlags = {
    "Afghanistan": { "mini": "http://flags.fmcdn.net/data/flags/mini/af.png", "normal": "http://flags.fmcdn.net/data/flags/normal/af.png" },
...
var pontos = 0;
const pontoTexto = document.querySelector('.pontos');
...
function getRandomCountry() {
    const countries = Object.keys(countriesFlags);
...
function setQuestion() {
    ...
    if (existentes.includes(currentCountry)) {
        setQuestion();
    } else {
        existentes.push(currentCountry)
        ...
        flagImg.onload = function () {
            // Esconde o spinner de carregamento quando a imagem é carregada
            carregando.style.display = 'none'
            flagImg.style.display = 'block'
        };


        flagImg.src = countriesFlags[currentCountry].normal;
        ...
        console.log(getCountryName(currentCountry))
        options.sort(() => Math.random() - 0.5);
        const buttons = document.querySelectorAll('.option');
        for (let i = 0; i < buttons.length; i++) {
            buttons[i].innerText = getCountryName(options[i]);
        }
        pontoTexto.innerText = pontos;
    }
}

function getCountryName(countryCode) {
    // Verifica se o país tem um alias definido
    if (countriesFlags[countryCode].alias) {
        return countriesFlags[countryCode].alias;
    } else {
        return countryCode; // Retorna o nome do país caso não tenha alias
    }
}
...
function checkAnswer(button) {
    ...
    console.log("Resposta: " + answer + " | Comparar: " + getCountryName(currentCountry))
    if (answer === getCountryName(currentCountry)) {
        ...
        saveHighScore(pontos);
        gameContainer.style.display = 'none'; // Oculta o container do jogo
        pontuacao.innerText = pontos + " bandeiras corretas!"
        perdeuDiv.style.display = 'flex'; // Exibe a div "perdeu"
    }
}
